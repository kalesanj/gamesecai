...
/* ——— Local Knowledge Base: instant answers even if Gemini is down ——— */
const KB = [
...
`Use a password manager for unique, strong passphrases. Never reuse passwords and enable MFA.` },
  { keys:["vpn"], answer:() =>
`A VPN encrypts traffic between your device and the company network. Use the approved client and keep it updated.` },
...
function scenarioExplainer(ctx="") {
  ...
  if (c.includes("usb")) return "It tests awareness of infected removable media. Safe action: give the USB to IT/security—never plug unknown media.";
...
// Version-flexible Gemini caller (v1 then v1beta), no “-latest”
